...
public class Core {
    public static void init(Env dEnv) {
        ...
        dEnv.addTokenProcedure("mod", ((args, env) -> {
            ...
            if (args.get(1).equals(DNumber.valueOf(0))) throw new DevoreRuntimeException("模数不能为0.");
            return DNumber.valueOf(((DInt) args.get(0)).toBigInteger().mod(((DInt) args.get(1)).toBigInteger()));
        }), 2, false);
        dEnv.addTokenProcedure("rem", ((args, env) -> {
            if (!(args.get(0) instanceof DInt)) throw new DevoreCastException(args.get(0).type(), "int");
            if (!(args.get(1) instanceof DInt)) throw new DevoreCastException(args.get(1).type(), "int");
            if (args.get(1).equals(DNumber.valueOf(0))) throw new DevoreRuntimeException("除数不能为0.");
        ...
        dEnv.addTokenProcedure("println", ((args, env) -> {
            ...
            for (DToken t : args) builder.append(t);
        ...
        dEnv.addAstProcedure("def-macro", ((ast, env) -> {
            ...
            for (int i = 1; i < ast.size(); ++i) nodes.add(ast.get(i).copy());
        ...
        dEnv.addAstProcedure("def", ((ast, env) -> {
            ...
            if (ast.get(0).isEmpty() && ast.get(0).type != Ast.Type.PROCEDURE) {
                ...
                env.addTokenProcedure(ast.get(0).symbol.toString(), ((cArgs, cEnv) -> {
                    ...
                    DToken result = DWord.NIL;
                    for (Ast node : nodes) result = Evaluator.eval(newEnv, node.copy());
                    return result;
                }), params.size(), false);
            }
            return DWord.NIL;
            ...
            for (Ast node : ast.get(0).children) {
                if ("apply".equals(node.symbol.toString())) {
                    ...
                    newEnv.addTokenProcedure(ast.get(0).symbol.toString(), ((cArgs, cEnv) -> {
                        ...
                        for (Ast tmp : res) inResult = Evaluator.eval(newInEnv, tmp.copy());
                        return inResult;
                    }), params.size(), false);
                } else {
        ...
        dEnv.addAstProcedure("lambda", ((ast, env) -> {
            ...
            for (int i = 1; i < ast.size(); ++i) node.add(ast.get(i).copy());
            BiFunction<Ast, Env, DToken> df = (inAst, inEnv) -> {
                ...
                DToken inResult = DWord.NIL;
                for (Ast tmp : node) inResult = Evaluator.eval(newInEnv, tmp.copy());
                return inResult;
            };
        ...
        dEnv.addTokenProcedure("++", ((args, env) -> {
            ...
            if (flag) {
                ...
                StringBuilder builder = new StringBuilder();
                for (DToken arg : args) builder.append(arg.toString());
